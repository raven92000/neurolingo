...
function EcranIntro({ mots, titreLecon, onStart }) {
  ...
  useEffect(() => {
    if (audioDebloque && motActuel) setTimeout(() => playWord(motActuel.en), 200)
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#090E1A', display: 'flex', flexDirection: 'column', padding: '0 20px 40px', maxWidth: '430px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '12px', fontWeight: '600', color: '#8B5CF6', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 6px' }}>Leçon suivante</p>
        <h1 style={{ fontFamily: 'Nunito, sans-serif', fontSize: '26px', fontWeight: '900', color: '#FFFFFF', margin: 0, lineHeight: 1.1 }}>{titreLecon}</h1>
      </div>
      ...
      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '11px', color: 'rgba(255,255,255,0.3)', textAlign: 'center', margin: '0 0 24px' }}>Mot {index + 1} sur {mots.length}</p>
      <div onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '24px', padding: '20px 0', minHeight: '380px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
          <h2 style={{ fontFamily: 'Nunito, sans-serif', fontSize: '44px', fontWeight: '900', color: '#FFFFFF', margin: 0 }}>{motActuel.en}</h2>
        ...
        <div style={{ background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: '20px', padding: '10px 22px' }}>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '18px', color: '#C4B5FD', margin: 0, fontWeight: '500' }}>{motActuel.fr}</p>
        </div>
      </div>
      {index === 0 && (<p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '13px', color: 'rgba(255,255,255,0.3)', textAlign: 'center', margin: '0 0 16px' }}>← Glisse pour découvrir le mot suivant →</p>)}
      <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
        ...
        {estDernier ? (
          <button onClick={onStart} style={{ flex: 1, height: '54px', background: 'linear-gradient(135deg, #7C3AED, #6D28D9)', color: '#FFFFFF', border: 'none', borderRadius: '16px', fontSize: '16px', fontFamily: 'Nunito, sans-serif', fontWeight: '800', cursor: 'pointer', boxShadow: '0 0 28px rgba(124,58,237,0.35)' }}>Commencer la leçon</button>
        ) : (
          <button onClick={swipeSuivant} style={{ flex: 1, height: '54px', background: 'rgba(139,92,246,0.12)', border: '1px solid rgba(139,92,246,0.25)', color: '#C4B5FD', borderRadius: '16px', fontSize: '15px', fontFamily: 'Nunito, sans-serif', fontWeight: '700', cursor: 'pointer' }}>Mot suivant →</button>
        )}
      </div>
    </div>
  )
...
function EcranExposition({ mot, etape, total, onNext }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#090E1A', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 20px 40px', maxWidth: '430px', margin: '0 auto' }}>
      ...
      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '12px', fontWeight: '600', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 48px' }}>Nouveau mot</p>
      <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'translateY(0) scale(1)' : 'translateY(12px) scale(0.97)', transition: 'all 0.4s ease', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '28px', width: '100%' }}>
        <div onClick={() => playWord(mot.en)} style={{ width: '160px', height: '160px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>{mot.svg}</div>
    ...
    <div style={{ minHeight: '100vh', background: '#090E1A', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 20px 40px', maxWidth: '430px', margin: '0 auto' }}>
      ...
      {feedback && (
        <div style={{ width: '100%' }}>
          ...
          <button onClick={() => showContinue && onNext(feedback === 'correct')} disabled={!showContinue} style={{ width: '100%', height: '54px', background: !showContinue ? 'rgba(255,255,255,0.06)' : feedback === 'correct' ? 'linear-gradient(135deg, #58CC02, #3DAD00)' : 'linear-gradient(135deg, #7C3AED, #6D28D9)', color: !showContinue ? 'rgba(255,255,255,0.25)' : '#FFFFFF', border: 'none', borderRadius: '16px', fontSize: '17px', fontFamily: 'Nunito, sans-serif', fontWeight: '800', cursor: showContinue ? 'pointer' : 'not-allowed', transition: 'all 0.4s ease' }}>Continuer</button>
          {feedback === 'wrong' && (<p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '12px', color: 'rgba(255,255,255,0.2)', textAlign: 'center', margin: '12px 0 0' }}>Chaque erreur te fait progresser.</p>)}
        </div>
      )}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#090E1A', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '0 20px 40px', maxWidth: '430px', margin: '0 auto' }}>
      ...
      <div style={{ width: '100%', opacity: showContinue ? 1 : 0, transform: showContinue ? 'translateY(0)' : 'translateY(8px)', transition: 'all 0.4s ease', pointerEvents: showContinue ? 'auto' : 'none' }}>
        <button onClick={() => onNext(true)} style={{ width: '100%', height: '54px', background: 'linear-gradient(135deg, #58CC02, #3DAD00)', color: '#FFFFFF', border: 'none', borderRadius: '16px', fontSize: '17px', fontFamily: 'Nunito, sans-serif', fontWeight: '800', cursor: 'pointer' }}>Continuer</button>
      </div>
    </div>
  )
}
